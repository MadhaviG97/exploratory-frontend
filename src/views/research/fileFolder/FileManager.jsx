...
function FileManager(props) {
  ...
  const handleSearch = (event) => {
    ...
    axios.post("/drive/searchfile", variables, config).then((response) => {
  ...
  const handleShare = () => {
    ...
    if (fileDetail.metadata.sensitivity == "private") {
      axios.post("/drive/sharefile", variable, config).then((response) => {
        if (response.data.success) {
          setFileShared(true);
          setTimeout(() => {
    ...
    if (fileDetail.metadata.sensitivity == "public") {
      axios.post("/drive/notsharefile", variable, config).then((response) => {
        if (response.data.success) {
  ...
  const handleDelete = () => {
    ...
    axios.post("/drive/softdeletefile", variable, config).then((response) => {
      if (response.data.success) {
        setFileDeleted(true);
        setTimeout(() => {
          window.location.reload();
  ...
  useEffect(() => {
    ...
    axios.post("/drive/getfolders", variable, config).then((response) => {
      if (response.data.success) {
    ...
    axios.post("/drive/getfiles", variable, config).then((response) => {
      if (response.data.success) {
        console.log(response.data.files);
        setFiles(response.data.files);
      } else {
    ...
    if (collabs.some((e) => e.researcher_id == user_id)) {
      return (
        <div>
          ...
          >
            <DialogContent>
              <DialogContentText>Delete File?</DialogContentText>
            </DialogContent>
            <DialogActions>
          ...
          >
            {fileDetail.metadata &&
            fileDetail.metadata.sensitivity == "private" ? (
              <MenuItem style={{ fontSize: 14 }} onClick={handleShare}>
                Share With Public
              </MenuItem>
            ) : (
              <MenuItem style={{ fontSize: 14 }} onClick={handleShare}>
            ...
            <MenuItem style={{ fontSize: 14 }} onClick={fileDownload}>
              Download
            </MenuItem>
            <MenuItem
              style={{ color: "#d60009", fontSize: 14 }}
                ...
                <Paper classname={classes.papermenu}>
                  <Box p={1.5}>
                    <FolderMenu
                      ...
                      folderParams={props.match.params}
                      group={group}
                    />
                  </Box>
                </Paper>
                  ...
                  <Grid container spacing={4} direction="row">
                    {folders.map((folder, index) => (
                      <Grid item lg={3} md={4} xs={8}>
                        <CardActionArea
                          component="a"
                          href={`/document/${group}/filemanager/${folder._id}`}
                        >
                          <Card>
                            <CardContent>
                              <div style={{ height: 140, marginBottom: 2 }}>
                                <img
                                  src={
                                    ...
                                    "/images/fileFolder/folderImage.png"
                                  }
                                  alt={folder.name}
                                />
                              </div>
                            ...
                            <CardContent>
                              <div style={{ height: 140, marginBottom: 2 }}>
                                <img
                                  src={
                                    process.env.PUBLIC_URL +
                                    "/images/fileFolder/fileImage3.png"
                                  }
