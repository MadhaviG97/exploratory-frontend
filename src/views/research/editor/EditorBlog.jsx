...
export default function CreatePage(props) {
  ...
  const handleSearch = (event) => {
    ...
    axios.post("/editor/searchblog", variables, config).then((response) => {
      if (response.data.success) {
        console.log(response.data.blogs);
        setBlogs(response.data.blogs);
      } else {
  ...
  const handleDelete = () => {
    ...
    axios.post("/editor/softdeletepost", variable, config).then((response) => {
      if (response.data.success) {
    ...
    if (collabs.some((e) => e.researcher_id == user_id)) {
      return (
        <div className={classNames(classes.main2)}>
          ...
          >
            <DialogContent>
              <DialogContentText>Delete Document?</DialogContentText>
            </DialogContent>
            <DialogActions>
                            ...
                            <CardActions disableSpacing>
                              <Tooltip title="Edit Document">
                                <IconButton
                                  ...
                                  href={`/document/${group}/edit/${blog._id}`}
                                >
                                  {/*href ={`/editor/delete/${blog._id}`} */}
                                  <EditIcon />
                                </IconButton>
                              </Tooltip>
                              <Tooltip title="Turn to PDF format">
                                <IconButton
                                  aria-label="settings"
                                  href={`/document/${group}/view/${blog._id}`}
                                >
                                  <PictureAsPdfIcon />
                                </IconButton>
                              </Tooltip>
